...
def solve(path):
    ...
    score[0,0] = 0
    start = [0,0,0,0] # i, j, cost, prev i, prev j
    end = [i - 1 for i in mat.shape]
    queue = [start]
    while len(queue) > 0:
        ij = queue.pop(0)
        for neigh in neighbors:
            i = ij[0] + neigh[0]
            j = ij[1] + neigh[1]
            if check_valid(i, j, mat) and (i != ij[2] or j != ij[3]): # don't got backwards
                new_score = score[ij[0], ij[1]] + mat[i, j]
                if new_score < score[i, j]:
                    score[i, j] = new_score
                    queue += [[i, j, ij[0], ij[1]]]
    print(score[end[0], end[1]])
    ...
    score = 1e15 * np.ones_like(bmat)
    score[0,0] = 0
    start = [0,0,0,0] # i, j, prev i, prev j
    end = [i - 1 for i in bmat.shape]
    queue = [start]
    while len(queue) > 0:
        ij = queue.pop(0)
        for neigh in neighbors:
            i = ij[0] + neigh[0]
            j = ij[1] + neigh[1]
            if check_valid(i, j, bmat) and (i != ij[2] or j != ij[3]):
                new_score = score[ij[0], ij[1]] + bmat[i, j]
                if new_score < score[i, j]:
                    score[i, j] = new_score
                    queue += [[i, j, ij[0], ij[1]]]
    print(score[end[0], end[1]])
